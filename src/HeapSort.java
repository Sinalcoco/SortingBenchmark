public class HeapSort extends SortingAlgorithm {
	...
	public void sort() {
		...
		// 1. Die Wurzel jedes Teilbaums ist größer, als jedes seiner Kinder
		// 2. Die Kinder einer Wurzel mit dem Index i sind an den Stellen (i+1)
		// * 2 - 1 für das linke, und (i+1) * 2 für das rechte Kind.
		// (i+1) ist nötig, da die Indize bei 0 beginnen anstatt bei eins, die
		// Formel aber für 1 beginnende Folgen gedacht ist.
		generateMaxHeap();

		// Sortiere das Array
		for (int i = helper.arrayLength() - 1; i > 0; i--) {
			// Die Wurzel (das größte Element) kommt ganz nach hinten im Array.
			// Dafür wird das ehemals letzte Element die neue Wurzel
			helper.swap(i, 0);
			// Die stelle i ist jetzt schon Teil des sortierten Arrays
			// Versenke die neue Wurzel an die passende Position im Heap.
			// Dadurch wird die Heap-Bedingung wiederhergestellt und das größte
	...
	private void versenke(int i, int n) {
		while (i <= (n / 2) - 1) {
			...
			if (kindIndex + 1 <= n - 1) {
				...
				// Jetzt steckt das größere Kind in der Variablen kindIndex;
			}

			// teste ob element sinken muss, also das Kind größer ist als die
			// Wurzel
			if (helper.compare(i, kindIndex) < 0) {
				...
				//Die Wurzel muss nicht mehr sinken und ist an der richtigen Position, also können wir aufhören
				break;
			}
		}
	}
